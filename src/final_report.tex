...
Cargo 採用 \textbf{DirtyReason} 機制來診斷變更。若計算出的指紋與磁碟上儲存的指紋不符，Cargo 會標記該 Unit 為 Dirty 並觸發 \texttt{JobQueue} 進行重編。
...
\begin{itemize}
    \item \textbf{sccache (Mozilla)}：透過包裝 rustc 編譯器，將編譯產物快取至本機或雲端。其限制在於無法快取 \textbf{連結階段 (Linking)}，且通常必須關閉增量編譯 (Incremental Compilation) 以確保跨機器的一致性。
...
\textbf{關鍵發現：}
\begin{enumerate}
    \item \textbf{Linking 並非主要瓶頸}：最終連結階段僅佔 4.3\% (5.9s / 138.8s)
    \item \textbf{Codegen 主導編譯時間}：程式碼生成階段佔據 86\% 的時間
